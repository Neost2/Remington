...
export default function PatientMessagesPage() {
  ...
  const send = async (e: FormEvent) => {
    ...
    if (!text.trim()) return
    if (mode === 'demo') {
      setMessages(prev => [...prev, { id: `demo-${Date.now()}`, channel: 'portal', direction: 'outbound', message: text, status: 'sent', relatedId: rideId, createdAt: new Date().toISOString(), user: { firstName: 'You', lastName: '', role: 'PATIENT' } }])
      setText(''); return
    }
    if (!token) { setError('Token required.'); return }
    setSending(true)
